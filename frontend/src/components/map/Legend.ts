    ...
    customLabels() {
      ...
      MapManager.sortDataByField(this.dataField)
        .then((data) => {
          ...
          const max = data.slice(-1)[0].cases;
          const range = heatLegend.valueAxis.axisRanges.getIndex(1);

          range.value = max;
          range.label.text = `${heatLegend.numberFormatter.format(max)}`;
        });

      this.chart.events.on("ready", () => {
        this.setLabelSize(minRange, maxRange);
        this.chart.events.on("sizechanged", () => this.setLabelSize(minRange, maxRange));
      });

      const label = this.chart.createChild(MapManager.libs.am4core.Label);
